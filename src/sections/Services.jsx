...
// 🔹 Beyonce-style Horizontal Scroll Gallery
const HorizontalGallery = ({ service, baseClass }) => {
  const scrollRef = useRef(null);

  if (!service.images || service.images.length === 0) return null;

  return (
    <div className={`${baseClass} bg-black overflow-hidden flex flex-col py-0 px-0 sm:px-0`}>
      <div className="px-6 sm:px-10 py-10">
        <span className="text-white/30 text-[10px] tracking-[6px] uppercase font-black mb-2 block">Archive</span>
        <h2 className="text-4xl sm:text-6xl font-black italic uppercase tracking-tighter text-white">{service.title}</h2>
      </div>
      ...
      >
        {service.images.map((img, i) => (
          <div 
            key={i} 
            className="flex-shrink-0 w-[85vw] sm:w-[60vw] md:w-[45vw] lg:w-[35vw] aspect-[4/5] sm:aspect-[3/4] rounded-[40px] overflow-hidden snap-center group relative"
          >
            <img 
              src={img} 
...
const Services = () => {
  ...
  // Sync with ScrollTrigger and Lenis
  useEffect(() => {
    if (!loading) {
  ...
  const openStream = (url, title) => {
    ...
    if (url.includes("youtube.com")) {
      ...
      finalUrl = url.includes("?") ? `${url}&autoplay=true` : `${url}?autoplay=true`;
    }
    ...
    <section id="weekly buzz" className="bg-black text-white w-full">
      ...
      <div className="flex flex-col w-full">
        {servicesData.map((service, index) => {
          const baseClass = "border-t border-white/10 px-6 sm:px-10 py-20 min-h-[60vh] relative w-full";
            ...
            return (
              ...
              >
                <div className="flex-1">
                  <h2 className="text-4xl sm:text-5xl font-black mb-6 uppercase italic tracking-tighter">{service.title}</h2>
                  <p className="text-lg text-white/60 leading-relaxed">{service.description}</p>
                </div>
                <div className="flex-1 w-full h-[450px]">
                  <img src={service.image} className="w-full h-full object-cover rounded-[32px] shadow-2xl" alt="" />
                </div>
              </div>
            );
          }

          // 🔹 FULL / HORIZONTAL GALLERY (Beyonce Style)
          if (service.type === "full") {
            return (
              <div key={service.id || index} className="w-full border-t border-white/10">
                <HorizontalGallery service={service} baseClass="relative w-full" />
              </div>
            );
          }
          ...
          if (service.type === "verse") {
            return (
              <div
                key={service.id || index}
                className={`${baseClass} relative flex flex-col items-center text-center py-40 w-full`}
              >
                <div 
                  className="absolute inset-0 bg-cover bg-center opacity-40"
                  style={{ backgroundImage: `url('${service.image || '/images/paradox.jpg'}')` }}
                />
                ...
                <div className="relative z-10 max-w-4xl px-6">
                  ...
                  <p className="text-white/40 text-sm uppercase tracking-widest">{service.description}</p>
                </div>
              </div>
            );
          }
          ...
          if (service.type === "video") {
            return (
              ...
              >
                <h2 className="text-5xl sm:text-7xl font-black italic uppercase tracking-tighter mb-16">{service.title}</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 w-full max-w-5xl">
                  {service.live_streams?.map((stream, sIdx) => {
                    const isYoutube = !!stream.youtube_id;
                    
                    // 🔹 PRIORITY: Custom Cover -> Youtube Thumb -> Placeholder
                    ...
                    return (
                      <div 
                        key={sIdx} 
                        onClick={() => openStream(isYoutube ? `https://www.youtube.com/embed/${stream.youtube_id}` : stream.facebook_url, stream.title)}
                      ...
                      >
                        <div className="relative bg-zinc-900/50 border border-white/10 rounded-[32px] overflow-hidden shadow-2xl">
                          <div className="aspect-video relative overflow-hidden">
                            <img src={thumb} className="w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all duration-700" alt="" />
                          ...
                          <div className="p-8">
                            <h4 className="font-bold text-xl mb-1 uppercase tracking-tight">{stream.title}</h4>
                            <p className="text-white/30 text-xs uppercase tracking-[3px] font-medium">{stream.schedule}</p>
                          </div>
                        </div>
                      </div>
                    );
                ...
                {service.discover_more_link && (
                  ...
                  >
                    Explore More
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="4" strokeLinecap="round" strokeLinejoin="round" className="group-hover:translate-x-1 transition-transform"><path d="M5 12h14m-7-7 7 7-7 7"/></svg>
                  </a>
                )}
              </div>
            );
      ...
      {/* Modal */}
      {modalOpen && (
        <div className="fixed inset-0 z-[200] bg-black/95 flex items-center justify-center p-4 backdrop-blur-xl" onClick={() => setModalOpen(false)}>
          <div className="w-full max-w-5xl aspect-video bg-black rounded-[40px] overflow-hidden border border-white/20 shadow-2xl relative" onClick={e => e.stopPropagation()}>
            <button 
            ...
            <iframe src={currentStream.url} className="w-full h-full border-0" allow="autoplay; fullscreen" />
          </div>
        </div>
      )}
    </section>
