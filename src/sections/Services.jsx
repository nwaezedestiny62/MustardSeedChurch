...
const Services = () => {
  ...
  return (
    <section id="weekly-buzz" className="bg-black text-white">
      ...
      {servicesData.map((service, index) => {
        ...
        if (service.type === "video") {
          return (
            ...
            >
              <video src={service.video || "/videos/program.mp4"} autoPlay muted loop playsInline className="absolute inset-0 w-full h-full object-cover" />
              ...
              <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full flex flex-col items-center">
                <div className="text-center mb-10 md:mb-16">
                  ...
                  <h2 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-bold tracking-tighter mb-4 px-2">{service.title}</h2>
                  <p className="text-lg sm:text-xl md:text-2xl text-white/70 max-w-2xl mx-auto px-2">{service.description}</p>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8 w-full max-w-5xl">
                  {service.live_streams?.map((stream, streamIdx) => {
                    ...
                    const embedUrl = isYoutube ? `https://www.youtube.com/embed/${stream.youtube_id}` : stream.facebook_url;
                    const thumbnail = isYoutube ? `https://img.youtube.com/vi/${stream.youtube_id}/maxresdefault.jpg` : "https://picsum.photos/id/1015/1280/720";
                    const badgeColor = isYoutube ? "bg-red-600" : "bg-sky-600";
                    return (
                      <div key={streamIdx} onClick={() => openStream(embedUrl, stream.title)} className="group relative cursor-pointer active:scale-95 transition-transform">
                        ...
                        <div className="relative bg-white/10 backdrop-blur-2xl border border-white/30 rounded-3xl overflow-hidden shadow-2xl">
                          <div className="aspect-video bg-black/80 relative overflow-hidden">
                            <img src={thumbnail} alt={stream.title} className="w-full h-full object-cover transition-transform group-hover:scale-105 duration-700" />
                ...
                {service.discover_more_link && (
                  <a href={service.discover_more_link} target="_blank" rel="noopener noreferrer" className="mt-10 md:mt-16 group inline-flex items-center gap-3 bg-white text-black font-semibold px-8 sm:px-10 py-4 rounded-2xl hover:bg-white/90 transition-all duration-300 hover:scale-105 active:scale-95 text-base sm:text-lg w-full sm:w-auto justify-center">
                    Discover More of Our Content
                    <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5 group-hover:translate-x-1 transition-transform duration-300" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}>
              ...
              <div className="absolute bottom-6 md:bottom-8 left-1/2 -translate-x-1/2 flex gap-2">
                {Array.from({ length: 3 }).map((_, i) => <div key={i} className="w-2 h-0.5 bg-white/40 rounded-full" />)}
              </div>
            </div>
          );
        }
      ...
      {/* ==================== LIVE STREAM MODAL ==================== */}
      {modalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/95 backdrop-blur-2xl p-3 sm:p-6" onClick={() => setModalOpen(false)}>
          <div className="relative w-full max-w-5xl mx-auto" onClick={(e) => e.stopPropagation()}>
            <div className="flex justify-between items-center mb-4 px-2">
              <h3 className="text-xl sm:text-2xl font-bold text-white pr-4 line-clamp-1">{currentStream.title}</h3>
              <button onClick={() => setModalOpen(false)} className="text-4xl leading-none text-white/70 hover:text-white transition-colors">×</button>
            </div>
            <div className="aspect-video bg-black rounded-2xl sm:rounded-3xl overflow-hidden border border-white/20 shadow-2xl">
              <iframe src={currentStream.url} className="w-full h-full" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; fullscreen" allowFullScreen title={currentStream.title} />
            </div>
            <p className="text-center text-white/50 text-sm mt-6">Tap outside or press ESC to close</p>
          </div>
        </div>
      )}
    </section>
