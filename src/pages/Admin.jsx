...
import { ACCEPT_MEDIA, isVideoUrl } from "../lib/media";
import {
  Plus, Save, Trash2, Edit2, X, Image as ImageIcon,
  Video, List, Lock, ShieldCheck, Key,
} from "lucide-react";

const MediaPreview = ({ src, className = "w-12 h-12 rounded-xl" }) => {
  if (!src) return null;
  if (isVideoUrl(src)) {
    return (
      <div className={`${className} relative overflow-hidden border border-white/10 shadow-lg bg-black`}>
  ...
  return <img src={src} className={`${className} object-cover border border-white/10 shadow-lg`} alt="" />;
};
...
const Admin = () => {
  ...
  const { uploadMedia, uploadMultipleMedia, uploading, progress, status, error: uploadError } =
    useMediaUpload("mscmedia");
  ...
  const handleMediaUpload = async (e, field) => {
    ...
    if (field === "images") {
      const urls = await uploadMultipleMedia(files, "services");
      if (urls.length) {
    ...
    const url = await uploadMedia(files[0], field === "video" ? "videos" : "services");
    if (!url) return;
    ...
    if (field === "video") {
      ...
      if (isVideoUrl(url)) {
        setFormData((prev) => ({ ...prev, video: url, image: prev.image }));
      } else {
  ...
  const handleStreamCoverUpload = async (e, index) => {
    ...
    e.target.value = "";
    if (!file) return;
    const url = await uploadMedia(file, "streams");
    if (!url) return;
    setFormData((prev) => {
      const newStreams = [...prev.live_streams];
      newStreams[index] = { ...newStreams[index], cover_image: url };
      return { ...prev, live_streams: newStreams };
    });
  };

  const handleStreamVideoUpload = async (e, index) => {
    const file = e.target.files?.[0];
    e.target.value = "";
    if (!file) return;
    const url = await uploadMedia(file, "streams");
    ...
    setFormData((prev) => {
      const newStreams = [...prev.live_streams];
      newStreams[index] = { ...newStreams[index], video_url: url };
      return { ...prev, live_streams: newStreams };
    });
  };
  ...
  const UploadBar = () =>
    uploading ? (
      <div className="space-y-2">
        <div className="flex justify-between text-[8px] font-black uppercase tracking-[2px] text-emerald-400">
    ...
    return (
      <div className="min-h-screen bg-black text-white flex items-center justify-center p-4 sm:p-6 font-sans overflow-hidden">
        <div className="w-full max-w-md bg-zinc-900 p-8 sm:p-10 rounded-[30px] sm:rounded-[40px] border border-white/10 shadow-2xl text-center relative">
          ...
          <h1 className="text-2xl sm:text-3xl font-black italic uppercase tracking-tighter mb-2 mt-8">
            Restricted Access
          </h1>
          <p className="text-white/40 text-[9px] uppercase tracking-[4px] font-bold mb-8">
            Mustard Seed Church Admin
          </p>
          <form onSubmit={handleEntryLogin} className="space-y-4">
            ...
            <button
              type="submit"
              className="w-full bg-white text-black font-black uppercase tracking-[4px] py-4 rounded-2xl hover:scale-105 active:scale-95 transition-all text-[10px]"
            >
              Unlock CMS
            </button>
          </form>
  ...
  const splitMedia = formData.video || formData.image;
      ...
      <div className="max-w-6xl mx-auto">
        <header className="flex flex-col sm:flex-row justify-between items-center gap-6 mb-12 sm:mb-16 text-center sm:text-left">
          <div className="w-full sm:w-auto">
            ...
            <p className="text-emerald-400 text-[8px] sm:text-[10px] tracking-[4px] sm:tracking-[6px] uppercase font-black mt-2">
              Images + Video · Supabase Storage
            </p>
          </div>
          <button
        ...
        <div className="space-y-6 sm:space-y-8 pb-20">
          ...
          {loading && services.length === 0 ? (
            <div className="py-20 sm:py-32 text-center text-white/20 uppercase tracking-[6px] sm:tracking-[8px] font-black text-[10px] animate-pulse">
              Syncing with Supabase...
            </div>
          ) : services.length === 0 ? (
            <div className="py-20 sm:py-32 text-center border-2 border-dashed border-white/5 rounded-[30px] sm:rounded-[50px] text-white/20 uppercase tracking-[6px] sm:tracking-[8px] font-black text-[10px]">
              No Content Created
            </div>
          ) : (
            <div className="grid grid-cols-1 gap-4 sm:gap-6">
              {services.map((service) => (
                <div
                  key={service.id}
                  className="group bg-zinc-900/40 border border-white/5 p-6 sm:p-8 rounded-[30px] sm:rounded-[40px] flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6 hover:bg-zinc-900/60 transition-all hover:border-white/10 shadow-lg"
                >
                  <div className="flex items-center gap-4 sm:gap-8 w-full lg:w-auto">
                    ...
                    <div className="min-w-0 flex-1">
                      <h3 className="text-xl sm:text-2xl font-black italic uppercase tracking-tighter mb-1 truncate">
                        {service.title}
                      </h3>
                      <div className="flex items-center gap-2 sm:gap-4 overflow-hidden">
                        <span className="flex-shrink-0 text-[7px] sm:text-[8px] font-black uppercase tracking-[2px] text-emerald-400 bg-emerald-400/10 px-2 py-0.5 rounded-full border border-emerald-400/20">
                          {service.type}
                        </span>
                        <p className="text-[9px] sm:text-[10px] text-white/30 truncate italic font-medium">
                          {service.description || service.verse || "Database Content"}
                        </p>
                      </div>
                    </div>
                  </div>
                  <div className="flex items-center gap-3 w-full lg:w-auto mt-2 lg:mt-0">
                    <button
                      ...
                      className="flex-1 lg:flex-none h-12 bg-white/5 hover:bg-white hover:text-black rounded-xl sm:rounded-2xl flex items-center justify-center transition-all border border-white/5 px-4 sm:px-6 font-black text-[9px] uppercase tracking-widest"
                    >
                      <Edit2 size={14} className="mr-2" /> Edit
                    </button>
                    <button
                      onClick={() => handleDeleteRequest(service.id)}
                      className="flex-1 lg:flex-none h-12 bg-red-900/10 hover:bg-red-600 text-red-500 hover:text-white rounded-xl sm:rounded-2xl flex items-center justify-center transition-all border border-red-900/10 px-4 sm:px-6 font-black text-[9px] uppercase tracking-widest"
                    >
          ...
          <div className="bg-zinc-900 w-full max-w-4xl min-h-screen sm:min-h-0 sm:rounded-[50px] border-x sm:border border-white/10 shadow-2xl relative flex flex-col">
            <div className="sticky top-0 bg-zinc-900/90 backdrop-blur-xl p-6 sm:p-10 border-b border-white/5 flex justify-between items-center z-20">
              <div className="min-w-0">
                ...
                <p className="text-[7px] sm:text-[9px] text-emerald-400 uppercase tracking-[3px] font-black mt-1">
                  Photos & videos welcome
                </p>
              </div>
              <button
                onClick={() => setShowModal(false)}
                className="flex-shrink-0 w-10 h-10 sm:w-14 sm:h-14 bg-white/5 hover:bg-white hover:text-black rounded-full flex items-center justify-center transition-all border border-white/10 ml-4"
              >
                <X size={20} />
              </button>
            </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 sm:gap-10">
                <div className="space-y-6 sm:space-y-8">
                  <div className="bg-black/20 p-6 rounded-[30px] border border-white/5 space-y-6">
                    <div>
                      <label className="text-[8px] font-black uppercase tracking-[2px] text-white/30 block mb-2">
                        Section Type
                      </label>
                      <select
                      ...
                      >
                        <option value="split">Split (Media + Text)</option>
                        <option value="full">Gallery (Photos & Videos)</option>
                        <option value="verse">Verse of the Week</option>
                        <option value="video">Live, Uploads & Streams</option>
                      </select>
                    </div>
                    <div>
                      <label className="text-[8px] font-black uppercase tracking-[2px] text-white/30 block mb-2">
                ...
                <div className="space-y-6 sm:space-y-8">
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap items-center gap-4">
                          <button
                            ...
                            disabled={uploading}
                            className="flex items-center gap-3 bg-white text-black px-5 py-3 rounded-xl font-black text-[8px] uppercase tracking-[1px] hover:scale-105 active:scale-95 transition-all disabled:opacity-50"
                          >
                            <ImageIcon size={14} /> {uploading ? "…" : "Upload photo / video"}
                          </button>
                          <input
                            type="file"
                            accept={ACCEPT_MEDIA}
                          ...
                          />
                          <MediaPreview src={splitMedia} className="w-20 h-20 rounded-xl" />
                        </div>
                        {splitMedia && isVideoUrl(splitMedia) && (
                          <p className="text-[9px] text-emerald-400 mt-3 uppercase tracking-widest">
                            Video attached — it will autoplay (muted) on the site
                          </p>
                        )}
                      </div>
                    </div>
                  ...
                  {formData.type === "full" && (
                    <div className="bg-black/20 p-6 rounded-[30px] border border-white/5 space-y-6">
                      <label className="text-[8px] font-black uppercase tracking-[2px] text-white/30 block mb-2">
                        Gallery — mix photos & videos
                      </label>
                      <button
                        type="button"
                        onClick={() => galleryInputRef.current.click()}
                        disabled={uploading}
                        className="w-full border-2 border-dashed border-white/10 rounded-2xl py-8 flex flex-col items-center gap-2 hover:border-emerald-500/50 transition-all bg-black/40 group disabled:opacity-50"
                      >
                        <ImageIcon
                          size={24}
                          className="text-white/20 group-hover:text-emerald-500/50 transition-colors"
                        />
                        <span className="text-[7px] font-black uppercase tracking-[1px] text-white/30">
                      ...
                      <div className="grid grid-cols-3 sm:grid-cols-4 gap-3 mt-4">
                        {formData.images.map((img, i) => (
                          <div
                            key={i}
                            className="relative group aspect-square rounded-xl overflow-hidden border border-white/5"
                          >
                            <MediaFrame
                              src={img}
                            ...
                            />
                            {isVideoUrl(img) && (
                              <span className="absolute top-1 left-1 text-[7px] font-black uppercase bg-black/70 px-1.5 py-0.5 rounded">
                                Video
                              </span>
                            )}
                            <button
                  ...
                  {formData.type === "verse" && (
                    <div className="bg-black/20 p-6 rounded-[30px] border border-white/5 space-y-6">
                      ...
                      <div>
                        ...
                        <button
                          type="button"
                          onClick={() => verseInputRef.current.click()}
                          disabled={uploading}
                          className="flex items-center gap-3 bg-white text-black px-5 py-3 rounded-xl font-black text-[8px] uppercase tracking-[1px]"
                        >
                        ...
                        />
                        <div className="mt-3">
                          <MediaPreview src={formData.video || formData.image} className="w-full h-28 rounded-xl" />
                        </div>
                      </div>
                    </div>
                  )}
                  ...
                  {formData.type === "video" && (
                    <div className="space-y-6">
                      <div className="bg-black/20 p-6 rounded-[30px] border border-white/5 space-y-4">
                        <label className="text-[8px] font-black uppercase tracking-[2px] text-white/30 block">
                          Featured uploaded video (plays on the site)
                        </label>
                        <button
                          type="button"
                          ...
                          className="flex items-center gap-3 bg-white text-black px-5 py-3 rounded-xl font-black text-[8px] uppercase tracking-[1px]"
                        >
                          <Video size={14} /> {uploading ? "Uploading…" : "Upload MP4 / WEBM"}
                        </button>
                        <input
                          type="file"
                      ...
                      <div className="bg-black/20 p-6 rounded-[30px] border border-white/5 space-y-6">
                        ...
                        <div className="space-y-4">
                          {formData.live_streams.map((stream, i) => (
                            <div
                              key={i}
                              className="bg-black border border-white/10 p-5 rounded-[25px] space-y-4 relative group shadow-2xl"
                            >
                              <button
                                type="button"
                                onClick={() =>
                                  setFormData({
                                    ...formData,
                                    live_streams: formData.live_streams.filter((_, idx) => idx !== i),
                                  })
                                }
                              ...
                              <div className="space-y-3">
                                ...
                                />
                                <input
                                  placeholder="YouTube ID or Facebook URL (optional if you upload a video)"
                                  className="w-full bg-zinc-900 border border-white/5 rounded-lg px-3 py-2 text-[10px] outline-none focus:border-emerald-500"
                                ...
                                <div className="flex items-center justify-between px-4 py-2 bg-zinc-900 border border-white/5 rounded-lg">
                                  <span className="text-[8px] font-black uppercase tracking-[1px] text-white/30">
                                    Live Badge?
                                  </span>
                                  <input
                                    type="checkbox"
                                ...
                                <div className="flex items-center gap-3 pt-2 flex-wrap">
                                  <label className="flex-1 cursor-pointer flex items-center justify-center gap-2 bg-white/5 border border-white/5 px-3 py-2 rounded-lg hover:bg-white/10 transition-all">
                                    ...
                                    <span className="text-[8px] font-black uppercase tracking-[1px]">
                                      Cover photo
                                    </span>
                                    <input
                                      type="file"
                                  ...
                                  <label className="flex-1 cursor-pointer flex items-center justify-center gap-2 bg-emerald-500/10 border border-emerald-500/20 px-3 py-2 rounded-lg hover:bg-emerald-500/20 transition-all">
                                    ...
                                    <input
                                      ...
                                      accept="video/mp4,video/webm,video/ogg,video/quicktime,.mp4,.webm,.mov"
                                      className="hidden"
                                      onChange={(e) => handleStreamVideoUpload(e, i)}
                                    />
                                  </label>
                                  {stream.cover_image && (
                                    <img
                      ...
                      <div className="bg-black/20 p-6 rounded-[30px] border border-white/5">
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setFormData({ ...formData, discover_more_link: e.target.value })
                          }
                        />
                      </div>
                    </div>
      ...
      {showGate && (
        <div className="fixed inset-0 z-[250] bg-black/98 flex items-center justify-center p-4 sm:p-6 backdrop-blur-3xl">
          <div className="w-full max-w-md bg-zinc-900 p-8 sm:p-12 rounded-[40px] sm:rounded-[50px] border border-white/10 shadow-[0_0_100px_rgba(0,0,0,0.5)] text-center relative">
            ...
            <h3 className="text-2xl sm:text-3xl font-black italic uppercase tracking-tighter mb-2 mt-8">
              Authorize {gateType.toUpperCase()}
            </h3>
            <p className="text-white/40 text-[8px] sm:text-[9px] mb-8 leading-relaxed uppercase tracking-[3px] font-bold">
            ...
            <form onSubmit={verifyGate} className="space-y-4 sm:space-y-6">
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                <button
                  type="submit"
                  className="flex-1 bg-white text-black font-black uppercase tracking-[3px] py-4 sm:py-5 rounded-2xl hover:scale-105 active:scale-95 transition-all text-[10px]"
                >
                  Confirm
                </button>
                <button
